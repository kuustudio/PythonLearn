...
def deal_xml(parent_tag, target_tag, value, target_list, content) ->dict:
    """
    @info：根据传入的 xml 字符串 以及标准tag的key value 得到目标tag和value 并返回
    :param parent_tag:  标准以及目标tag的父级tag
    :param target_tag:  标准tag的key
    :param value:  标准tag的value
    :param target_list:  目标tag的key的列表集合
    :param content: xml字符串
    :return: 目标的tag和内容的集合 map  形式
    :rtype: dict
    """
    except_value = {}
    except_rule = None
    try:
        ...
        rule_tags = collection.getElementsByTagName(parent_tag)
        if rule_tags and isinstance(rule_tags,list):
            for one_rules in rule_tags:
                target_content = one_rules.getElementsByTagName(target_tag)[0].childNodes[0].data
                if target_content and target_content == value:
                    except_rule = one_rules
                    break
        if except_rule:
            for one_except in target_list:
                get_value_list = except_rule.getElementsByTagName(one_except)[0].childNodes
                if get_value_list:
                    if len(get_value_list)>1:
                        value_list = [one.data for one in get_value_list]
                        except_value[one_except] = value_list
                    else:
                        except_value[one_except] = get_value_list[0].data
...
if __name__ == '__main__':
    x = test_request()
    print(x)
    y = deal_xml("ruleerror","gzbs","A30481111",['msg','sentence'],x)
    z = deal_xml("ruleerror","gzbs","P2005",['msg','sentence'],x)
    print(y)
    print(z)
