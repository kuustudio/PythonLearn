...
def deal_xml(path, **kwargs):
    target_tag = None
    path_list = deal_path(path)
    if path_list:
        for item in path_list:
            parent = item[0]
            target = item[1]
            pd  = re.compile(r'[[](.*)[]]', re.S)
            target_index = re.findall(pd,parent)

def deal_path(path):
    deal_path_list = []
    temp_path_list = path.split(".")
    if temp_path_list:
        len_temp_list = len(temp_path_list)
        for n_temp in range(0, len_temp_list + 1):
            one = []
            if temp_path_list[n_temp + 1:]:
                one.append(temp_path_list[n_temp])
                one.append(temp_path_list[n_temp + 1])
                deal_path_list.append(one)


    return deal_path_list
